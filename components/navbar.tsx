...
export function Navbar() {
  ...
  // Do not render Navbar on the /payme page
  if (pathname === "/payme") {
    return null;
  }
        ...
        <div className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`relative transition-colors duration-300 ${isScrolled
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                ...
                <span className="hidden sm:inline">{localeLabels?.find((e) => e.value === lang)?.name}</span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              {localeLabels?.map((item) => (
                <DropdownMenuItem
                  key={item?.value}
                  onClick={() => { fetchTranslations(item?.value); setLang(item?.value); fetchSiteinfo(item?.value) }}
                  className={lang === item?.value ? "bg-accent" : ""}
                >
                  {item?.name}
                </DropdownMenuItem>
              ))}
